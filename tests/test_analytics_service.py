...
def test_collective_individual_and_opponent_kpis_with_fixture() -> None:
    ...
    assert collective["points_total"] == 5
    assert collective["goals_total"] == 3
    assert collective["points_per_possession"] == 1.25
    assert collective["goals_per_possession"] == 0.75
    assert collective["offensive_conversion_rate"] == 1.0
    assert collective["technical_error_rate"] == 0.25
    assert collective["defensive_stops_per_possession"] == 0.25
    ...
    assert collective["set_performance"] == {
        ...
        2: {"points_total": 2, "goals_total": 1, "technical_errors": 0},
    }
    ...
    assert individual[2]["shot_attempts"] == 2
    ...
    assert opponent["preferred_attack_side"] == "left"
    assert opponent["most_frequent_shooter_player_id"] == 99
    assert opponent["top_two_point_scorer_player_id"] == 99
    assert opponent["pressure_error_rate"] == 0.25
    assert opponent["transition_vulnerability"] == 0.5
